...
Renderer::Renderer(){
	...
	if (Mix_OpenAudio(MIX_DEFAULT_FREQUENCY, MIX_DEFAULT_FORMAT, 2, 1024) == -1) throw "Unable to intit sdlmixer audio systems";
	soundtrack = Mix_LoadMUS((PATH_AU + MENUTHEME).c_str());
...
void Renderer::SelectMusic(std::string type) {
	if (type == "menu")
		soundtrack = Mix_LoadMUS((PATH_AU + MENUTHEME).c_str());
	else if (type == "game")
		soundtrack = Mix_LoadMUS((PATH_AU + GAMETHEME).c_str());
	if (!soundtrack)throw "unable to load mix_music soundtrack";
}
...
void Renderer::ResumeMusic() {
	Mix_ResumeMusic();
}

void Renderer::MuteMusic() {
	Mix_VolumeMusic(0);
}

void Renderer::UnmuteMusic() {
	Mix_VolumeMusic(MIX_MAX_VOLUME / 2);
}
